...
rm -rf "${OUTPUT_DIR}"/minimax

fileList1=`cd ${INCLUDE_DIR} && find -name "*.h" | tail -n +1 | cut -c 3-`
fileList2=`cd ${LIB_DIR} && find -name "*.h" | tail -n +1 | cut -c 3-`

# Include all files in $INCLUDE_DIR in one minimax header file.
cat "${DIR}/license.h" >> "${OUTPUT_DIR}"/minimax
echo -e "\n#pragma once\n" >> "${OUTPUT_DIR}"/minimax
for file in $fileList1
do
    echo "#include \"${SUB_HEADERS_PREFIX}/${file}\"" >> "${OUTPUT_DIR}"/minimax
done
